...
{  
	...
	DEBUG(0, ("dbg: allocation_size = %d, file_len = %d\n", fsp->initial_allocation_size, file_len));
	
	/* Save the requested allocation size. */
	if ((info == FILE_WAS_CREATED) || (info == FILE_WAS_OVERWRITTEN)) {
	DEBUG(0, ("dbg0: allocation_size = %d, file_len = %d\n", allocation_size, file_len));
		allocation_size = (SMB_BIG_UINT)IVAL(inbuf,smb_ntcreate_AllocationSize);
	DEBUG(0, ("dbg11: allocation_size = %d, file_len = %d\n", allocation_size, file_len));

#ifdef LARGE_SMB_OFF_T
		allocation_size |= (SMB_BIG_UINT)(((SMB_BIG_UINT)IVAL(inbuf,smb_ntcreate_AllocationSize + 4)) << 32);

#endif
	DEBUG(0, ("dbg22: allocation_size = %d, file_len = %d\n", allocation_size, file_len));
		if (allocation_size && (allocation_size > (SMB_BIG_UINT)file_len)) {
			fsp->initial_allocation_size = (SMB_BIG_UINT)smb_roundup((SMB_BIG_UINT)fsp->conn, (SMB_BIG_UINT)allocation_size);
			if (fsp->is_directory) {
				close_file(fsp,ERROR_CLOSE);
				END_PROFILE(SMBntcreateX);
			...
			fsp->initial_allocation_size = (SMB_BIG_UINT)smb_roundup((SMB_BIG_UINT)fsp->conn,(SMB_BIG_UINT)file_len);

		DEBUG(0, ("dbg21: allocation_size = %d, file_len = %d\n", allocation_size, file_len));
		}
	DEBUG(0, ("dbg33: allocation_size = %d, file_len = %d\n", allocation_size, file_len));
	}
	...
	DEBUG(0, ("allocation_size = %d\n", allocation_size));
	SOFF_T(p, 0, allocation_size);
	p += 8;
	SOFF_T(p,0,file_len);
	p += 8;
	if (flags & EXTENDED_RESPONSE_REQUIRED) {
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	DEBUG(0, ("dbg0: fsp->conn = %d, file_len = %d\n", fsp->conn, file_len));
	...
	DEBUG(0, ("dbg1: allocation_size = %d, file_len = %d\n", allocation_size, file_len));
#ifdef LARGE_SMB_OFF_T
		allocation_size |= (SMB_BIG_UINT)(((SMB_BIG_UINT)IVAL(params,16)) << 32);
	DEBUG(0, ("dbg2: allocation_size = %d, file_len = %d\n", allocation_size, file_len));
#endif
	...
	DEBUG(0, ("dbg3: allocation_size = %d, file_len = %d\n", allocation_size, file_len));
	}
