...
static BOOL file_is_dir(connection_struct *conn, char *name, SMB_STRUCT_STAT *pst)	// J++
{
        /* If we can't stat it does not show it */
        if (!VALID_STAT(*pst) && (SMB_VFS_STAT(conn, name, pst) != 0))
                return False;
...
BOOL is_sys_dir(const char *const dirname)
{											// J++ modified from libdisk
        ...
        for (i = 0; MS_System_folder[i] != NULL; ++i) {
                if (strequal(dirname, MS_System_folder[i]))
                        return True;
        }

        for (i = 0; Linux_System_folder[i] != NULL; ++i) {
                if (strequal(dirname, Linux_System_folder[i]))
                        return True;
        }
...
BOOL is_visible_file(connection_struct *conn, const char *dir_path, const char *name, SMB_STRUCT_STAT *pst, BOOL use_veto)
{
	...
	/* J++ */
	char *entry_t = NULL;        
	if (asprintf(&entry_t, "%s/%s", dir_path, name) != -1)
	{
		if (file_is_dir(conn, entry_t, pst) && is_sys_dir(name))
		{
			SAFE_FREE(entry_t);
			return False;
		}
