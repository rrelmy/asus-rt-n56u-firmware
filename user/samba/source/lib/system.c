...
struct passwd *sys_getpwnam(const char *name)
{
 	return brcm_getpwnam(name);
	/* NOTE(GEW): We cannot get 'getpwnam()' to work properly, so I have
	   coded my own version. */
}

struct passwd *sys_getpwuid(uid_t uid)
{
	return brcm_getpwuid(uid);
	/* NOTE(GEW): We cannot get 'getpwuid()' to work properly, so I have
	   coded my own version. */
}
...
{
	return brcm_getgrnam(name);
	/* NOTE(vkp): We cannot get 'getpwuid()' to work properly, so I have
	   coded my own version. */
}

struct group *sys_getgrgid(gid_t gid)
{
	return brcm_getgrgid(gid);
	/* NOTE(vkp): We cannot get 'getpwuid()' to work properly, so I have
	   coded my own version. */
}
