					    ...
					    struct winbindd_cli_state *state)
{
	...
	pw = sys_getpwuid(state->request.data.uid);
	if (pw == NULL) {
					    ...
					    struct winbindd_cli_state *state)
{
	...
	pw = sys_getpwnam(state->request.data.username);
	if (pw == NULL) {
		return WINBINDD_ERROR;
	}
					    ...
					    struct winbindd_cli_state *state)
{
	...
	gr = sys_getgrgid(state->request.data.gid);
	if (gr == NULL)
		return WINBINDD_ERROR;
					    ...
					    struct winbindd_cli_state *state)
{
	...
	gr = sys_getgrnam(state->request.data.groupname);
	if (gr == NULL) {
		return WINBINDD_ERROR;
	}
