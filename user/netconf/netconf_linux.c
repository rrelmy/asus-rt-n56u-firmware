 ...
 *
 * $Id: netconf_linux.c,v 1.1.1.1 2007/02/15 12:14:15 jiahao Exp $
 */
...
static const char * ipt_nat_chain_name[] = { 
	...
	"POSTROUTING", "VSERVER", "POSTROUTING"		// upnp add
#ifdef NETCONF_NETMAP
, NULL
#endif
};
...
target_num(const struct ipt_entry *entry, iptc_handle_t *handle)
{
	...
	if (strncmp(name, "DROP", IPT_FUNCTION_MAXNAMELEN) == 0)
		...
		return NETCONF_APP;
#ifdef CONFIG_NETMAP
	else if (strncmp(name, "NETMAP", IPT_FUNCTION_MAXNAMELEN) == 0)
		return NETCONF_NETMAP;
#endif
	else
		return -1;
}
...
insert_entry(const char *chain, struct ipt_entry *entry, iptc_handle_t *handle)
{
	...
	if (!iptc_get_target(entry, handle))	// J++
	{
