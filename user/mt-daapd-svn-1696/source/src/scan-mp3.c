...
#ifdef ALPHA_CUSTOMIZE //2009.08.17
#include "charguess.h"
#endif
...
#ifdef ALPHA_CUSTOMIZE //2009.08.17
int IsBIG5Symbol(unsigned char *p)
{
	if((*p == 0xA1 || *p == 0xA2 || *p == 0xA3) && *(p+1) >= 0x40 && *(p+1) <0x7f)
		return 1;

	if((*p == 0xA1 || *p == 0xA2) && *(p+1) >= 0xA1 &&  *(p+1) <=0xFE)
		return 1;

	if( *p==0xA3 && *(p+1) >= 0xA1 &&  *(p+1) <=0xBF)
		return 1;
	else
		return 0;
}

int LongToUChar(unsigned char * out_str, unsigned long * native_text)
{
	int len = 0;
	for(;;)
	{
		if(*native_text != 0x0000L)
		{
			if (*native_text <= 0x000000ffL) 
			{
				out_str[len]=*native_text;
				len++;
			}
			else if (*native_text <= 0x0000ffffL)
			{
				out_str[len]=*native_text >> 8;
				len++;
				out_str[len]=*native_text & 0x00ff;
				len++;
			}
		}
		else
...
int CodePageType()
{
	...
	if((fp = fopen("/etc/codepage", "r")) == NULL) 
	{
		return ENG;	//vincent20070601
	}
	memset(string,0,sizeof(string));
	fgets(string,24,fp);
	fclose(fp);
	
	if((strstr(string,"cp950"))!=0) //BIG5
	return BIG5;
	else if((strstr(string,"cp936"))!=0) //GB
	return GB;
	else if((strstr(string,"cp932"))!=0) //Japan
	return JPN;
	else if((strstr(string,"cp949"))!=0) //Koren
	return KOR;
	else
	return ENG;
...
int IsUniCodeSymbol(unsigned long *p)
{
	do
	{
		if(*p >=0x4E00 && *p <=0x9FBF)
		{
			return 1; // getting UniCode symbol
		}
		p++;
	}while(*p != 0x00L);
...
#ifdef ALPHA_CUSTOMIZE //2009.08.17
    unsigned char char_type_str[100];
    char *szDate = NULL;
    char old_szDate[20] = {0};
    unsigned char in_str[2048];
    unsigned char *in_str_ptr = NULL;
    int in_str_len;
    id3_ucs4_t *ptr;
    int iconv_again = 0;
...
#ifdef ALPHA_CUSTOMIZE //2009.08.17
		iconv_again = 0;
		memset(char_type_str, 0x00, sizeof(char_type_str));
		ptr = (id3_ucs4_t*) native_text;
		in_str_ptr = in_str;
		memset(in_str, 0x00, sizeof(in_str));
		in_str_len = LongToUChar(in_str_ptr, ptr);
		strncat(char_type_str, in_str, sizeof(char_type_str));

		//printf("pid3frame->id =[%s]\n", pid3frame->id);
	cover_again:
		if (szDate == NULL)
			szDate = GuessChardet(char_type_str);
		...
		/*
		else if (strcmp(szDate, "ASCII") != 0)
		{
			memset(old_szDate, 0x00, sizeof(old_szDate));
			strcpy(old_szDate, szDate);
		}
		*/
		/*
		if (!szDate)
		...
		*/
		...
		if(!strcmp(szDate,"Big5") || !strcmp(szDate,"IBM855") || !strcmp(szDate,"x-euc-tw"))
			conversion_codepage = strdup("CP950");
		else if (!strcmp(szDate, "windows-1251") || !strcmp(szDate,"IBM866") || !strcmp(szDate,"x-mac-cyrillic"))
			conversion_codepage = strdup("ISO-8859-1");
		else if (!strcmp(szDate, "windows-1252"))
		{
			conversion_codepage = strdup("Big5");
			szDate = "Big5";
		}
		else if (!strcmp(szDate,"ISO-8859-7"))
			conversion_codepage = strdup("CP932");
		//else if (!strcmp(szDate, "gb18030"))
		//	conversion_codepage = strdup("GB2312");
		else
			conversion_codepage = strdup(szDate);

		//printf("szDate = [%s]\n", szDate);

		free(utf8_text);
                utf8_text =
                    (char*)util_xtoutf8_alloc(char_type_str,
                                              strlen(char_type_str),
                                              conversion_codepage);
		if (utf8_text == NULL)
		{
			if (iconv_again == 0)
			...
			{
				if(!strcmp(pid3frame->id,"TPE1") || !strcmp(pid3frame->id,"TCOM"))
					utf8_text = strdup("");
				else if(!strcmp(pid3frame->id,"TIT2"))
					utf8_text = strdup(pmp3->fname);
				else
					utf8_text = (char*)id3_ucs4_utf8duplicate(native_text);
			}
		}
...
#ifndef ALPHA_CUSTOMIZE //2009.08.17
    free(conversion_codepage);
#endif
