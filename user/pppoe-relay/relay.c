...
* LIC: GPL
*
* $Id: relay.c,v 1.1.1.1 2007/02/15 12:14:31 jiahao Exp $
*
***********************************************************************/
...
// pre check, tmp test
	...
	printf("HAVE_STRUCT_SOCKADDR_LL\n");		// tmp test
#else
	printf("NOT HAVE STRUCT_SOCKADDR_LL\n");	// tmp test
#endif
//
    /* Enter the relay loop */
	printf("enter relay loop\n");	// tmp test
    relayLoop();
	printf("end relay loop\n");	// tmp test
...
relayLoop()
{
	...
	printf("discovery sock is %d\n", sock); // tmp test

	if (sock > maxFD) maxFD = sock;
	FD_SET(sock, &readable);
	sock = Interfaces[i].sessionSock;

	printf("session sock is %d\n", sock);   // tmp test

	if (sock > maxFD) maxFD = sock;
	FD_SET(sock, &readable);
	if (CleanPipe[0] > maxFD) maxFD = CleanPipe[0];
	
	printf("clean pipe[0] is %d\n", CleanPipe[0]);  // tmp test
