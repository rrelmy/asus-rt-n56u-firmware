...
"$Id: if.c,v 1.1.1.1 2007/02/15 12:14:31 jiahao Exp $";
...
void 
printPacket(PPPoEPacket *pkt, int sock, int size)
{
        int i, j;
        char buf[size+1];

	printf("[sock:%d]\n", sock);
        dumpPacket(stderr, pkt, "SEND");        // tmp test     
        printf("\n");   // tmp test
...
int
test_send(int sock, PPPoEPacket *pkt, int size)
{
	int bufsize = size + 40;
	char buf[bufsize];
	int i, j;
...
{
	//printPacket(pkt, sock, size);	// tmp test
#if defined(USE_BPF)
    if (write(sock, pkt, size) < 0) {
	sysErr("write (sendPacket)");
	return -1;
    }
#elif defined(HAVE_STRUCT_SOCKADDR_LL)	// send way here
	//printf("send size is %d, packet size is %d\n", size, sizeof(PPPoEPacket));	// tmp test

    	//int pkt_len = ntohs(pkt->length);	// ham try
    	//if(pkt_len < 13)	
	//{
    	//	test_send(sock, pkt, size);
	//} 
	//else
	//{
...
/*
        printf("[R:sock:%d]\n", sock);      // tmp test
	//flush(stdout);
        dumpPacket(stderr, pkt, "RECV");        // tmp test     
        printf("\n");   // tmp test
*/
    return 0;
}
