...
static unsigned char* get_utf8_text(const id3_ucs4_t* native_text) {
  ...
  if (lang_index == 1)
    ...
    rc = do_convert("CP936", "UTF-8", in8, strlen(in8), iconv_buf, MAX_ICONV_BUF);
  ...
  if (rc == ICONV_OK) {
	...
	rc = do_convert("UTF-8", "CP950", in, strlen(in), iconv_buf, MAX_ICONV_BUF);
      else if (lang_guess == 2)
	rc = do_convert("UTF-8", "CP936", in, strlen(in), iconv_buf, MAX_ICONV_BUF);
    }
    else
    {
    /* (2) try codepage1 -> utf8 */
      if (lang_index == 1)
	rc = do_convert("UTF-8", "CP950", in, strlen(in), iconv_buf, MAX_ICONV_BUF);
      else if (lang_index == 2)
	rc = do_convert("UTF-8", "CP936", in, strlen(in), iconv_buf, MAX_ICONV_BUF);
	...
	/* utf-8 including non-chinese char? fallback. */
...
void make_composite_tags(MP3FILE *song)
{
	...
	if (iconv_buf) {
	    if (lang_index == 1)
		rc = do_convert("UTF-8", "CP950", song->fname, strlen(song->fname), iconv_buf, MAX_ICONV_BUF);
	    else if (lang_index == 2)
		rc = do_convert("UTF-8", "CP936", song->fname, strlen(song->fname), iconv_buf, MAX_ICONV_BUF);

	    if (rc == ICONV_OK) {
		song->title = iconv_buf;
	    } else {
		song->title = strdup("UNKNOWN");
		free(iconv_buf);
	    }
	}
    } // if utf-8
