...
int db_add_playlist(unsigned long int playlistid, char *name, int file_time, int is_smart) {
	...
	if (iconv_buf) {
	    if (lang_index == 1)
		rc = do_convert("UTF-8", "CP950", name, strlen(name), iconv_buf, MAX_ICONV_BUF);
	    else if (lang_index == 2)
		rc = do_convert("UTF-8", "CP936", name, strlen(name), iconv_buf, MAX_ICONV_BUF);

	    if (rc == ICONV_OK) {
		pnew->name = iconv_buf;
	    } else {
		pnew->name = strdup("UNKNOWN");
		free(iconv_buf);	      
	    }
	}
    } // if utf-8
