...
void lease_update_file(time_t now, int force)
{
  struct dhcp_lease *lease;
  time_t next_event;
  int i, err = 0;
  ...
  if ((file_dirty != 0 || force != 0) && daemon->lease_stream)
    {
      errno = 0;
...
#endif
*/
	  elapsed_t = difftime(lease->expires, now);
	  ourprintf(&err, "%lu ", (unsigned long)(elapsed_t > 0 ? elapsed_t: 0));
