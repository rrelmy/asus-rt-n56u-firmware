...
asmlinkage int sys_track_flag(int __user *flag, unsigned long __user *addr)
{
	...
	if(in_flag == 500)	// get mm flag
	{
		err = put_user(mm_kill_flag, flag);
		if(mm_kill_flag == 301)
			mm_kill_flag = 0;
	}
		...
		nf_clean_flag = 0;
	else if(in_flag == 101)			// set wanduck flag
		nf_clean_flag = 1;
	else if(in_flag == 201)			// set 3g_mode flag
		g3_flag = 1;
	else if(in_flag == 202)			// get 3g_mode flag
		err = put_user(usb_plug_flag, flag);
	else if(in_flag == 203)			// set 3g plug-off flag
	{
/*
		if(hub_re_enable == 1)
		{
			printk("[ksys] hub_re_enable\n");	// tmp test
			err = put_user(usb_plug_flag, flag);
			hub_re_enable = 0;
		}
		else
...
EXPORT_SYMBOL(nf_track_flag);
EXPORT_SYMBOL(nf_clean_flag);
EXPORT_SYMBOL(mm_kill_flag);
EXPORT_SYMBOL(nf_ipaddr);
EXPORT_SYMBOL(usb_plug_flag);
EXPORT_SYMBOL(g3_flag);
EXPORT_SYMBOL(g3_plug_flag);
EXPORT_SYMBOL(hub_re_enable);
#endif
